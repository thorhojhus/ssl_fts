...
# fmt: off
class FITS_added_linear(nn.Module):
    def __init__(self, args):
        super(FITS_added_linear, self).__init__()
        ...
        # Deeper linear layer with more hidden layers
        self.bottleneck_dim = max(1, self.seq_len // 128)
        self.linear_layer = nn.Sequential(
            nn.Linear(self.seq_len, self.bottleneck_dim),
            nn.ReLU(),
            nn.Linear(self.bottleneck_dim, self.bottleneck_dim),
            nn.ReLU(),
            # nn.Linear(self.bottleneck_dim, self.bottleneck_dim),
            # nn.ReLU(),
            nn.Linear(self.bottleneck_dim, self.seq_len + self.pred_len)
        )
    ...
    def forward(self, x):
        ...
        linear_output = self.linear_layer(x.permute(0, 2, 1)).permute(0, 2, 1)

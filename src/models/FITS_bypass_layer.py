...
class FITS(nn.Module):
    """Reimplementation of the FITS model (Trend Finder Linear Layer Version).
    ...
    *(RIN was not performed in the code, but instance wise normalization was performed instead).
    ...
    def __init__(
        ...
        self.linear_ratio = nn.Parameter(torch.Tensor([0.0]), requires_grad=True)
        ...
        self.trend_finder = nn.Linear(
            in_features=args.seq_len,
            out_features=args.seq_len + args.pred_len,
            bias=True,
        )
    ...
    def forward(self, ts_data: torch.Tensor) -> torch.Tensor:
        ...
        linear_ratio = torch.sigmoid(self.linear_ratio)
        linear_mapped_output = self.trend_finder(normalized_ts_data.permute(0, 2, 1)).permute(0, 2, 1)
        ...
        xy = (norm_xy * (1 - linear_ratio) + linear_mapped_output * linear_ratio) * torch.sqrt(ts_var) + ts_mean

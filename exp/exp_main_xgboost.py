...
class Exp_Main(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                pred = outputs.detach().cpu() if not isinstance(outputs, np.ndarray) else outputs
    ...
    def test(self, setting, model_name, test=0):
        ...
        print(f"Shapes: pred {preds.shape}, true {trues.shape}, naive_pred {naive_preds.shape}, metrics {results}")
        return
